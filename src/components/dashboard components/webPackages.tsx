...
type SerializableWebPackage = Omit<
  WebPackage,
  "addons"
> & {
  addons: SerializableAddon[];
  icon?: never;
};

type Props = {
  packageData: SerializableWebPackage;
};
...
/*
 * Package icons stay entirely inside this Client Component.
 *
 * They are NEVER sent from a Server Component.
 */
const packageIconMap: Record<
  PackageSlug,
  LucideIcon
> = {
  launch: Globe2,
...
/*
 * Resolve an add-on icon from plain string data.
 *
 * This means PackageAddon no longer needs to send
 * a Lucide component across the Server → Client boundary.
 */
function getAddonIcon(
  addon: SerializableAddon
): LucideIcon {
  const value =
    `${addon.id} ${addon.title}`.toLowerCase();
            ...
            {scopeOpen && (
              <>
                <p className="mt-3 max-w-[540px] text-[11px] leading-5 text-black/42">
                  Select anything your
                  project may need. The
                  estimate updates
                ...
                <div className="mt-6 grid gap-3 md:grid-cols-2">
                  {packageData.addons.map(
                    (addon) => {
                       ...
                       * IMPORTANT:
                       * Resolve icon HERE.
                       * addon itself contains only
                       * serializable strings/numbers.
                       */
                      const AddonIcon =
                        getAddonIcon(
                          addon
