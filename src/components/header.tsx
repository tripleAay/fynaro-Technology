...
export default function DashboardHeader({
  ...
  const { items } = useCart(); // items inferred as CartItem[]
  ...
  // Load client name from prop or localStorage
  useEffect(() => {
    if (userName) {
      setName(userName);
      setLoading(false);
  ...
  const cartCount = useMemo(() => {
    return items?.reduce((total: number, item: CartItem) => total + (item.quantity ?? 1), 0) ?? 0;
  }, [items]);
    ...
    <>
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-black">{name || "User"}</span>
            </div>
      ...
      <SlideOver open={messagesOpen} onClose={closeAllPanels}>
        <div className="flex h-full flex-col">
          ...
          <p className="text-xs text-black/50">Hook your inbox / chat UI here.</p>
        </div>
      </SlideOver>
...
function RowLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 rounded-xl px-3 py-2 hover:bg-black/5 transition">
